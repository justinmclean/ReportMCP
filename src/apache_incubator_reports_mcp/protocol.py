    ...
    if method == "initialize":
        return jsonrpc_result(
            message_id,
            {
                ...
                "serverInfo": {"name": "apache-incubator-reports-mcp", "version": "0.1.0"},
                "capabilities": {"tools": {}},
            },
        )
...
def parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Apache Incubator Reports MCP server")
    parser.add_argument("--cache-dir", help="Path to cache ASF Incubator reports")
    parser.add_argument("--repo-url", help="ASF Incubator reports repository URL")
